  ...
  return (
    <>
                                ...
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) => {
                                    return checked
                                      ? field.onChange([
                                          ...field.value,
                                          {subject,id:academicYearSubjects?.find(a=>a.subjectId===subject.id)?.id||cuid()},
                                        ] satisfies AcademicYearSubjectSchema[])
                                      : field.onChange(
                                          field.value.filter(
                                            (d) => d.subject.id !== subject.id,
                                          ),
                                        );
                                  }}
                                />
      ...
      <SheetFooter className="sticky bottom-0 flex flex-row items-center gap-2">
       ...
       <Button type="button" variant={'default'} disabled={!form.formState.isDirty} onClick={()=>setOpenConfirmDialog(true)}>Update</Button>

      </SheetFooter>
      <Dialog open={openConfirmDialog} onOpenChange={setOpenConfirmDialog}>
<DialogContent>
  <DialogHeader>
    <DialogTitle><TriangleAlert className="inline"/> Attention</DialogTitle>
    <DialogDescription>Dangerous action, continue with caution</DialogDescription>
  </DialogHeader>
  <p>Please note that <strong>unchecking</strong> the already existing(checked) subjects means deleting them. When they are deleted, all the <strong>exams</strong> under them together with the <strong>pupils/ students' exam scores</strong> shall be deleted as well.</p>
  <DialogFooter>
<Button type="button" variant={'outline'} onClick={()=>setOpenConfirmDialog(false)}>Go back</Button>
 <LoadingButton
 type="submit"
 variant={'destructive'}
          size={"default"}
