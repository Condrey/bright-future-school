...
export default function ListOfItems({ oldItem }: ListOfItemsProps) {
  ...
  const {
    ...
    queryKey,
    queryFn: async () => getIndividualGeneralStoreItems(oldItem.id),
    initialData: oldItem.individualGeneralStoreItems,
  });
  if (status === "error") {
    console.error(error);
  }
