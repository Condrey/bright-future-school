...
interface ItemBodyProps {
  oldItem: IndividualGeneralStoreItemData;
}
...
export default function ItemBody({ oldItem }: ItemBodyProps) {
  ...
  const {
    ...
    queryKey,
    queryFn: async () => getIndividualGeneralStoreItem(oldItem.id),
    initialData: oldItem,
  });
  if (status === "error") {
    console.error(error);
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-3">
            <div className="flex flex-col">
              <CardTitle>{item.generalStoreItem.name}</CardTitle>
            </div>
            ...
            <div className="w-full space-y-4">
              ...
              <div>
                <div className="flex items-start">
                  ...
                  <p>{item.generalStoreItem.asset.name}</p>
                </div>
                <p className="font-semibold italic">
                  Category:{" "}
                  {assetCategories[item.generalStoreItem.asset.category].label}
                </p>
                <TipTapViewer
                  content={item.generalStoreItem.asset.description}
                />
              </div>
            </div>
          )}
