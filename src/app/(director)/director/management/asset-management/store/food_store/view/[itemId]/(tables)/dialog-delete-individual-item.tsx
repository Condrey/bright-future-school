...
interface DeleteIndividualItemDialogProps {
  item: IndividualFoodStoreItemData;
  open: boolean;
...
export default function DialogDeleteIndividualItem({
  ...
  const mutation = useDeleteItemMutation(item.foodStoreItemId);

  async function handleDeletion() {
    try {
      mutation.mutate(
        { id: item.id, foodStoreItemId: item.foodStoreItemId },
        {
          onSuccess() {
            openChange(false);
          },
  ...
  return (
    <ResponsiveDrawer
      open={open}
      setOpen={openChange}
      title={`Delete ${item.foodStoreItem.foodName} with ${item.uniqueIdentifier || "unknown"} unique uniqueIdentifier`}
