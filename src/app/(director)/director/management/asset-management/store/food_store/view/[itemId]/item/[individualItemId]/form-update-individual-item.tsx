...
interface FormUpdateIndividualItemProps {
  individualItemToEdit: IndividualFoodStoreItemData;
  open: boolean;
...
export default function FormUpdateIndividualItem({
  ...
  const form = useForm<IndividualFoodStoreSchema>({
    resolver: zodResolver(individualFoodStoreSchema),
    values: {
      id: individualItemToEdit.id || "",
      foodStoreItemId: individualItemToEdit.foodStoreItemId || "",
  ...
  function handleFormSubmission(input: IndividualFoodStoreSchema) {
    mutate(input, {
      onSuccess() {
        setOpen(false);
      },
  ...
  return (
    <ResponsiveDrawer
      title={`Update this ${individualItemToEdit.foodStoreItem.foodName}`}
      setOpen={setOpen}
      open={open}
    >
      <Form {...form}>
