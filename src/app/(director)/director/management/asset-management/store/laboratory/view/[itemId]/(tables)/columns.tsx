...
export const useItemColumn: ColumnDef<IndividualLaboratoryItemData>[] = [
  {
    id: "index",
    header({ column }) {
  ...
  {
    accessorKey: "labItem.name",
    header({ column }) {
      return <DataTableColumnHeader column={column} title="Items" />;
    },
    cell: ({ row }) => {
      const laboratoryItem = row.original.labItem;
      return (
        <div>
          <div>{laboratoryItem.name}</div>
        </div>
      );
    },
  },
