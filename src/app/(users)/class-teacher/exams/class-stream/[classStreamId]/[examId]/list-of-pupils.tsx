...
export default function ListOfPupils({ examAndPupils }: ListOfPupilsProps) {
  ...
  if (status === "success" && !data.exam?.examSubjects.length)
    return (
      <EmptyContainer
        message={
  ...
  const baseFields = ["id", "pupil", "examSubjects", "agg", "position"];
  const tableData = createTableRows({
    examScores: data?.exam?.examSubjects.flatMap((s) => s.examScores)!,
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-lg">
            <YearContainer year={year} /> {className} {stream}{" "}
            <span className="block text-xs sm:inline sm:text-lg">
              ({exam.examName} - {exam.classTerm.term?.term})
            </span>
          </h1>
